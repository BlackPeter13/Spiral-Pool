...
" "$sentinel_cfg" "$final_api_key" 2>/dev/null || true
    ...
    # --- v2.3.0: Fix payments disabled for coins added via dashboard/pool-mode ---
    ...
    if grep -q "enabled: false" "$CONFIG_FILE" 2>/dev/null; then
        ...
        if [[ "$payment_fixes" -gt 0 ]]; then
            ...
            log_info "  - Fixed $payment_fixes coin(s) with payments disabled (v2.3.0 bug fix)"
...
#
# v2.3.0 and earlier shipped dbcache=8192 for SHA-256d coins and dbcache=4096
...
echo -e "                                     ${DIM}V2.3.0 - PHI HASH REACTOR${NC}"
echo ""
...
embed = {
    ...
    "color": 0xFF6B35,
    "footer": {"text": "Spiral Pool v2.3.0 — Phi Hash Reactor  •  coin-upgrade.sh handles the chain resync risk"}
}
...
SSHDEOF
    ...
    # v2.3.0: Reduce oversized dbcache/maxconnections in existing daemon configs.
    # Multi-coin setups with dbcache=8192 per daemon exhaust RAM and cause RPC
    # timeouts that stall block confirmation.
    rightsize_daemon_resources

    # v2.3.0: Patch daemon service files with ExecStartPre ownership fix.
