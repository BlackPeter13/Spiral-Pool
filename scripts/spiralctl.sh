    ...
    case "$action" in
        status)
            ...
            if systemctl is-active --quiet patroni 2>/dev/null; then
                ...
                local patroni_json
                patroni_json=$(curl -s --connect-timeout 3 --max-time 5 http://localhost:8008/patroni 2>/dev/null)
        ...
        promote)
            ...
            if systemctl is-active --quiet patroni 2>/dev/null; then
                ...
                patroni_status=$(curl -s --connect-timeout 3 --max-time 5 http://localhost:8008/patroni 2>/dev/null)
...
ha_enable() {
    ...
    if [[ "$ha_mode" == "ha-master" ]]; then
        ...
        echo -e "  Patroni:    $(curl -sf http://localhost:8008/ 2>/dev/null | grep -oP '"role"\s*:\s*"\K[^"]+' || echo 'initializing...')"
        echo ""
    fi
    ...
    if [[ -n "$peer_ips" ]]; then
        ...
        while IFS= read -r pip; do
            ...
            if sudo -u "$POOL_USER" ssh -o BatchMode=yes -o ConnectTimeout=5 \
                ...
                sudo -u "$POOL_USER" ssh -o BatchMode=yes -o ConnectTimeout=10 \
                    "${POOL_USER}@${pip}" \
                    "curl -sf -X POST 'http://localhost:8008/reload' 2>/dev/null" \
                    2>/dev/null || true
